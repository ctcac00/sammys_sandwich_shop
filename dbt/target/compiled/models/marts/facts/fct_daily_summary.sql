...
with orders as (
    select * from SAMMYS_SANDWICH_SHOP.DBT_DEV_intermediate.int_orders
    where order_status = 'Completed'
),

order_items as (
    select * from SAMMYS_SANDWICH_SHOP.DBT_DEV_intermediate.int_order_items
),
...
select
    -- Surrogate key
    md5(cast(coalesce(cast(dm.order_date as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(dm.location_id as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as daily_summary_id,
